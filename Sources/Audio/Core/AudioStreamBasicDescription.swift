...
//    Pads the structure out to force an even 8 byte alignment.
    ...
    public init<T: AudioStreamType>(type: T.Type, sampleRate: Float64, format: AudioFormatID, channels: UInt32 = 1) {
        self.init(
            mSampleRate: sampleRate,
            mFormatID: format,
            mFormatFlags: type.formatFlags,
            mBytesPerPacket: type.bytesPerPacket,
            mFramesPerPacket: type.framesPerPacket,
            mBytesPerFrame: type.bytesPerFrame,
            mChannelsPerFrame: type.channelsPerFrame,
            mBitsPerChannel: type.bitsPerChannel,
            mReserved: 0
        )
    }
